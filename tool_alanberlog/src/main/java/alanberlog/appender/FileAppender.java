...
public class FileAppender extends AbsAppender {
    ...
    public static class Builder {
        ...
        private String getDefaultBufferPath(Context context) {
            File bufferFile;
            if (Environment.getExternalStorageState().equals(android.os.Environment.MEDIA_MOUNTED) && context.getExternalFilesDir("alanberLog") != null) {
                bufferFile = context.getExternalFilesDir("alanberLog");
            } else {
                bufferFile = new File(context.getFilesDir(), "alanberLog");
            }
            if (bufferFile != null && !bufferFile.exists()) {
                bufferFile.mkdirs();
            }
            return new File(bufferFile, ".alanberLogCache").getAbsolutePath();
        }
    }
}
