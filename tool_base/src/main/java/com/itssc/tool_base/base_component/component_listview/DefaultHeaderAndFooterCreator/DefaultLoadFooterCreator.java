package com.itssc.tool_base.base_component.component_listview.DefaultHeaderAndFooterCreator;
...
import com.itssc.tool_base.R;
import com.itssc.tool_base.base_component.component_listview.PullToLoad.LoadFooterCreator;
import com.itssc.tool_base.base_component.component_listview.PullToLoad.PullToLoadRecyclerView;
...
public class DefaultLoadFooterCreator extends LoadFooterCreator {
    ...
    public boolean onStartPull(float distance, int lastState) {
        if (lastState == PullToLoadRecyclerView.STATE_DEFAULT) {
            iv.setImageResource(R.mipmap.ptl_arrow_down);
            iv.setRotation(-180f);
            tv.setText("上拉加载");
    ...
    public void onStartLoading() {
        iv.setImageResource(R.mipmap.ptl_loading);
        startLoadingAnim();
        tv.setText("正在加载...");
    }
    ...
    public View getLoadView(Context context, RecyclerView recyclerView) {
        if (mLoadView == null) {
            mLoadView = LayoutInflater.from(context).inflate(R.layout.layout_pull_refresh_head,recyclerView,false);
    ...
    protected View getNoMoreView(Context context, RecyclerView recyclerView) {
        if (mNoMoreView == null) {
            mNoMoreView = LayoutInflater.from(context).inflate(R.layout.layout_pull_refresh_head,recyclerView,false);
